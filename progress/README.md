...
- **F1 fixed 2026-09-20.** `CRON_SECRET` set on Preview and Production. On the preview, the cron returned `{"checked":1,"failed":0}` with the bearer and 401 without it; the session it examined was already fulfilled, so nothing changed. Production keeps answering 401 until commerce is switched on, because `isCommerceConfigured` is checked before the bearer — expected, not a regression.
- **F2 fixed 2026-09-20** via a separate `staging` dataset. `VITE_SANITY_DATASET` is now two variables, production→`production` and preview→`staging`. Verified by setting `stock-mustard` to 5 in staging only: the preview reported `available:true` and `available:5` while the production dataset stayed at 0. A near miss worth remembering — the variable was first *edited* rather than split, which silently pointed production at staging too; nothing broke only because `VITE_*` is baked in at build time.
- **F3 fixed 2026-09-21.** `http://localhost:5173` was deleted and re-added with `--no-credentials`, and the stale `https://atelier-product-page-38z417igl-…` origin removed. Verified against the Sanity API: that origin now receives `Access-Control-Allow-Origin` with **no** `Access-Control-Allow-Credentials`, while `http://localhost:3333` still returns `Access-Control-Allow-Credentials: true` as the Studio requires. `http://localhost:3000` and `https://kimie-atelier.vercel.app` are credential-free too.
