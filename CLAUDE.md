...
- **Sanity Studio datasets**: `studio/sanity.config.ts` exports **two workspaces**, `production` (real catalogue and stock) and `staging` (what the branch preview reads and writes, so test purchases never touch live inventory). They are served at `/production` and `/staging`; bare `localhost:3333` redirects to the first. Add schema or plugin changes inside the shared `workspace()` factory so both get them, and keep its return type annotated `WorkspaceOptions` — without it the `document.actions` and `newDocumentOptions` callbacks lose their contextual typing and `tsc` fails. The CLI (`sanity.cli.ts`, `SANITY_STUDIO_DATASET`) still defaults to `production`; pass `--dataset` to target staging.
